...
export function useCommunity(selectedCommunityId?: string) {
  ...
  const [displayCommunities, setDisplayCommunities] = useState<Community[]>(communityService.getDisplayCommunities());

  useEffect(() => {
    const unsub = communityService.subscribe((list) => {
      setCommunities(list);
      setDisplayCommunities(communityService.getDisplayCommunities());
    });
    return unsub;
  }, []);
