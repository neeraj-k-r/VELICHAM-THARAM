...
class CommunityService {
  ...
  /**
   * Display-oriented view that clubs fragmented community records into one
   * entry per physical locality, using the same clustering rules as the
   * consensus engine. Counts are summed, member counts are summed, and the
   * worst active status (VERIFIED_OUTAGE > REPORTING > NORMAL) wins so a
   * verified outage in any fragment lights up the whole clubbed row.
   */
  public getDisplayCommunities(): Community[] {
    const all = this.getAllCommunities();

    // Every community maps to its cluster representative (highest member count).
    const repOf = new Map<string, string>();
    all.forEach(comm => {
      const nearby = this.getNearbyCommunityIds(comm.id);
      let repId = comm.id;
      let best = comm.memberCount || 0;
      nearby.forEach(id => {
        const cand = this.communities.get(id);
        if (cand && (cand.memberCount || 0) > best) {
          best = cand.memberCount || 0;
          repId = id;
        }
      });
      repOf.set(comm.id, repId);
    });

    const rank = (s?: OutageStatus) =>
      s === 'VERIFIED_OUTAGE' ? 2 : s === 'REPORTING' ? 1 : 0;

    const merged = new Map<string, Community>();
    all.forEach(comm => {
      const repId = repOf.get(comm.id);
      if (!repId) return;
      let rep = merged.get(repId);
      if (!rep) {
        rep = { ...(this.communities.get(repId) as Community) };
        merged.set(repId, rep);
      }
      if (comm.id !== repId) {
        rep.memberCount = (rep.memberCount || 0) + (comm.memberCount || 0);
        rep.activeReportsCount = (rep.activeReportsCount || 0) + (comm.activeReportsCount || 0);
        rep.activeRestoresCount = (rep.activeRestoresCount || 0) + (comm.activeRestoresCount || 0);
        if (rank(comm.status) > rank(rep.status)) {
          rep.status = comm.status;
          rep.lastOutageVerifiedAt = comm.lastOutageVerifiedAt ?? rep.lastOutageVerifiedAt;
          rep.lastRestoredAt = comm.lastRestoredAt ?? rep.lastRestoredAt;
        }
      }
    });
  ...
  /**
   * Find the existing community a new registration with the given locality name
   * should join, instead of creating yet another fragmented copy. Matching is
   * done on the normalized locality name; district mismatches are tolerated
   * when either side carries the generic "Kerala" fallback.
   */
  public getExistingCommunityIdByName(name: string, district?: string): string | null {
    const target = normalizeLocalityName(name);
    ...
    this.communities.forEach(comm => {
      const cName = normalizeLocalityName(comm.name);
      if (cName === target && cName.length > 2) {
        const cDist = (comm.district || '').trim().toLowerCase();
        const sameDist =
          dist === cDist ||
          dist === '' || cDist === '' ||
          dist === 'kerala' || cDist === 'kerala';
        if (sameDist && (comm.memberCount || 0) > bestMembers) {
          bestMembers = comm.memberCount || 0;
          bestId = comm.id;
        }
      }
    });
  ...
  /**
   * Resolve the community id a user profile should use: an existing locality
   * with the same name wins over the freshly computed candidate id, so repeated
   * registrations of the same place join one shared community.
   */
  public canonicalCommunityId(name: string, district: string, pincode: string, candidateId: string): string {
    return this.getExistingCommunityIdByName(name, district) || candidateId;
  }
  ...
  public async registerCustomUserCommunity(
    ...
    // Club onto an already-registered locality with the same name instead of
    // creating yet another fragmented community (different PIN/district
    // spellings currently produce duplicate cards in the grid).
    const existingId = this.getExistingCommunityIdByName(cleanName, district);
    if (existingId) {
      const existing = this.communities.get(existingId);
