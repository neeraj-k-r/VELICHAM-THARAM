 ...
 *
 * The Firebase credentials are passed at runtime from the build-time env
 * config, so no API key is ever hardcoded in the Android sources.
 */
export async function enableBackgroundTorch(communityId: string): Promise<boolean> {
  if (!isNativePlatform()) return false;
  try {
    await BackgroundTorchNative.start({
      communityId,
