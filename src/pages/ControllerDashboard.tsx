...
export const ControllerDashboard: React.FC = () => {
  ...
  const { communities, displayCommunities, resetAllToNormal } = useCommunity();
  ...
  useEffect(() => {
    if (displayCommunities.length > 0 && (!selectedCommId || !displayCommunities.find(c => c.id === selectedCommId))) {
      setSelectedCommId(displayCommunities[0].id);
    }
  }, [displayCommunities, selectedCommId]);
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-lg space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400 font-mono">
            {displayCommunities.length} Registered Hubs
          </span>
        </div>
