...
export const AuthPage: React.FC = () => {
  ...
  const handleStandardSubmit = async (e: React.FormEvent) => {
    ...
    const rawId = localityName.trim().toLowerCase().replace(/[^a-z0-9]/g, '_').slice(0, 24) + '_' + (pincode.slice(-4) || 'hub');
    // Club onto an already-registered locality with the same name so repeated
    // registrations of the same place don't spawn duplicate community cards.
    const cleanId = communityService.canonicalCommunityId(localityName, district, pincode, rawId);
