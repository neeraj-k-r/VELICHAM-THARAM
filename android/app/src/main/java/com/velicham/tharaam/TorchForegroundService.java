...
public class TorchForegroundService extends Service {
    ...
    // Firebase credentials arrive at runtime from the web layer's build-time
    // env config (no keys hardcoded in the sources).
    private static volatile boolean sConfigWarned = false;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent.hasExtra(EXTRA_API_KEY)) {
            ...
            getPrefs().edit().putString("apiKey", apiKey).apply();
        }
        if (intent.hasExtra(EXTRA_PROJECT_ID)) {
            projectId = intent.getStringExtra(EXTRA_PROJECT_ID);
            getPrefs().edit().putString("projectId", projectId).apply();
        }
    ...
    private void restoreState() {
        ...
        apiKey = getPrefs().getString("apiKey", "");
        projectId = getPrefs().getString("projectId", "");
    ...
    private void pollOnce() throws Exception {
        // Without Firebase credentials the service stays idle (app running in
        // local/demo mode) — never attempts the network.
        if (apiKey == null || apiKey.isEmpty() || projectId == null || projectId.isEmpty()) {
            if (!sConfigWarned) {
                sConfigWarned = true;
                Log.w(TAG, "Firebase configuration missing — background torch polling disabled");
                sLastError = "firebase config missing";
            }
            return;
        }
